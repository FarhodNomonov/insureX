...
function NatureOffice() {
  const navigate = useNavigate();
  const { hash } = useLocation();
  let personData = JSON.parse(localStorage.getItem("insured_person")) ?? {};
  let reportStorage = JSON.parse(localStorage.getItem("natureDamageOffice"));

  const [currentPage, setCurrentPage] = React.useState(
    Number(localStorage.getItem("nature_office_page") ?? 1) ?? 1
  );
  ...
  React.useInsertionEffect(() => {
    localStorage.setItem("nature_office_page", currentPage);
    reset();
  }, [currentPage]);
  ...
  React.useInsertionEffect(() => {
    if (
      ...
      localStorage.setItem("nature_office_page", 1);
    }
  }, []);
    ...
    <AccidentForm className="flex__column__ h-100">
      ...
      <div>
        <Header
          ...
          title={"פתיחת אירוע רכוש - נזקי טבע"}
        />
        {currentPage < 4 && (
      ...
      <FormBefore
        ...
        setIsLoading={setIsLoading}
        firstPage={4}
        storageName={"natureDamageOffice"}
        storagePage={"nature_office_page"}
        pageName={"נזקי טבע"}
        suppliyerType={3}
      />
            ...
            <ModalContent>
              ...
              <ModalBody style={{ marginBottom: "0" }}>
                ...
                <Button
                  onClick={() => {
                    if (currentPage === 5) {
                      localStorage.removeItem("natureDamageOffice");
                      localStorage.removeItem("nature_office_page");
                      report = { report: {} };
                      setDrawing(null);
                    } else {
                      postRequest(
